...
public:
  GBRForestCont(const std::string& ebFilename,
...
const std::pair<const GBRForestD*,const GBRForestD*>& 
GBRForestCont::operator()(double et,bool isEB) const {
  if(!isEB) {
    // Never apply EE — return a dummy pair of nullptrs
    static const std::pair<const GBRForestD*,const GBRForestD*> nullPair{nullptr,nullptr};
    return nullPair;
  }
  if(et < highEtThres_)    return eb_;
  else                     return ebHighEt_;
}
...
int main(int argc, char** argv)
{
 ...
 // cmdLineInt.addOption("gbrForestFileEEHighEt",gbrFilenameEEHighEt,"","gbrForestFile for endcap high et, highEtThres must be set for this to be read");
  ...
  GBRForestCont gbrForests(gbrFilenameEB,
			   gbrFilenameEBHighEt,
			   highEtThres);
 ...
 // const auto regDataEEAll = HistFuncs::readTree(inTree,varsEE+":"+*targetEE,"");
  ...
  fillTree(regDataEBAll,evtData,gbrForests,outTreeData,outTree,nrThreads);
	      ...
	      unsigned int nrThreads)
{
  ...
  auto initThread = [&](unsigned int threadNr) {
  unsigned int entryNr = threadEntryNrs[threadNr];
  // bounds check
  if (entryNr >= evtData.size()) return false;
  // only schedule EB entries (evtData[][3] == isEB)
  if (!evtData[entryNr][3]) {
    // skip EE
    return false;
  }
  // barrel entry: pick EB regData and EB forest
  auto& regData   = regDataEB[entryNr];
  auto  gbrForest = gbrForests(evtData[entryNr][4], /*isEB=*/true);
  ...
  while(nrActiveThreads!=0){
    ...
    for(unsigned int threadNr=0;threadNr<nrActiveThreads;threadNr++){
      ...
      outTreeData.invTar = 1.0 / regDataEB[entryNr].back();
      outTree->Fill();	
