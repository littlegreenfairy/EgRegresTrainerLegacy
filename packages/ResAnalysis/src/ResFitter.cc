...
ResFitter::Param ResFitter::makeCBFit(TH1* hist,float xmin,float xmax,const std::string& fitVarName)const
{
  ...
  model.fitTo(data,RooCmdArg("FitOptions",0,0,0,0,"mh",0,0,0),RooFit::Optimize(0),RooFit::Timer(1));
  auto fitRes = model.fitTo(data,RooCmdArg("FitOptions",0,0,0,0,"mh",0,0,0),RooFit::Optimize(0),RooFit::Timer(1));
...
ResFitter::Param ResFitter::makeCruijffFit(TH1* hist,float xmin,float xmax,const std::string& fitVarName)const
{
  ...
  model.fitTo(data,RooCmdArg("FitOptions",0,0,0,0,"mh",0,0,0),RooFit::Optimize(0),RooFit::Timer(1),RooFit::PrintEvalErrors(-1));
